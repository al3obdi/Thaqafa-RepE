...
PROVENANCE_MARKER = "<!-- provenance: live-model-run"


def check_provenance(results_md: str) -> None:
    """Refuse a results summary that does not come from a live model run.

    ``run_full_experiment`` stamps every summary it writes with a provenance
    comment. A summary without it was produced some other way - hand-edited,
    truncated, or generated by an older tool version that could fabricate
    placeholder numbers - and must not flow into the paper unnoticed.

    Args:
        results_md: Contents of RESULTS_SUMMARY.md.

    Raises:
        SystemExit: If the marker is missing.
    """
